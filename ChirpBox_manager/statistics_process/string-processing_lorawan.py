...
def str_to_dec_4byte( ori_string ):
    target_str = ''
    for i in range(0,len(ori_string)-8,8):
        ...
        integ_1 = int(ori_string[i+6:i+8],16) * 65536 + int(ori_string[i+8:i+10],16) * 16777216
        chara = str(integ)
        chara_1 = str(integ_1)
        target_str = target_str + chara + ','
        if i >= percent_count:
...
Round_Count = 0
with open("lorawan\lorawan_"+"6"+".txt","w") as f_target:

    for node in range(total_node_num):
        if (node!=5):
            #open original txt 
            # f_original = open("./sf7_12nodes(1F1173)_3.txt")
            f_original = open("lorawan/collect_"+"6"+".txt")
            ...
            for i in range(0,NUMBER_MESG-1):
                ...
                Message.LINE = str(int(Mesg_splitted[LENGTH_FILE_NAME*2 : (LENGTH_FILE_NAME+1)*2],16) + int(Mesg_splitted[(LENGTH_FILE_NAME+1)*2 : (LENGTH_FILE_NAME+2)*2],16)*256)
                Message.ARGUMENT = str_to_ascii(Mesg_splitted[(LENGTH_FILE_NAME+LENGTH_LINE)*2 : (LENGTH_FILE_NAME+LENGTH_LINE+LENGTH_AGRUMENT)*2])[:10]
                ...
                Message.VALUE = str_to_dec_4byte(Mesg_splitted[(LENGTH_FILE_NAME+LENGTH_LINE+LENGTH_AGRUMENT)*2 : (LENGTH_FILE_NAME+LENGTH_LINE+LENGTH_AGRUMENT+LENGTH_VALUE)*2])
                # Message.VALUE_1 = str_to_dec_4byte(Mesg_splitted[(LENGTH_FILE_NAME+LENGTH_LINE+LENGTH_AGRUMENT+4)*2 : (LENGTH_FILE_NAME+LENGTH_LINE+LENGTH_AGRUMENT+LENGTH_VALUE)*2])
                # wirte to file

                f_target.write("\tFile name: \t\t"+Message.FILE_NAMGE+"\n")
                f_target.write("\tLine: \t\t"+Message.LINE+"\n")
                f_target.write("\tArgument: \n"+Message.ARGUMENT+"\n")
                f_target.write("Value:"+Message.VALUE+"\n")
                # f_target.write("Value_1:"+Message.VALUE_1+"\n")
                f_target.write("\n")
            Count = 0
            mesg_original = ''
            f_original.close()
