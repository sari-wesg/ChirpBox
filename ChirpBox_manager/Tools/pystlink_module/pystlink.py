...
import argparse
import time
import pylib.stlinkusb
import pylib.stlinkv2
import pylib.stm32
import pylib.stm32fp
import pylib.stm32fs
import pylib.stm32l0
import pylib.stm32l4
import pylib.stm32h7
...
class PyStlink():
    ...
    def find_mcus_by_core(self):
        ...
        if cpuid == 0:
            raise pylib.stlinkex.StlinkException('Not connected to CPU')
        self._dbg.verbose("CPUID:  %08x" % cpuid)
        partno = 0xfff & (cpuid >> 4)
        for mcu_core in pylib.stm32devices.DEVICES:
            if mcu_core['part_no'] == partno:
                self._mcus_by_core = mcu_core
                return
        raise pylib.stlinkex.StlinkException('PART_NO: 0x%03x is not supported' % partno)
        ...
        for idcode_reg in idcode_regs:
            ...
            for mcu_devid in self._mcus_by_core['devices']:
                if mcu_devid['dev_id'] == devid:
                    ...
                    return
        raise pylib.stlinkex.StlinkException('DEV_ID: 0x%03x is not supported' % devid)
    ...
    def find_mcus_by_flash_size(self):
        ...
        if not self._mcus:
            raise pylib.stlinkex.StlinkException('Connected CPU with DEV_ID: 0x%03x and FLASH size: %dKB is not supported. Check Protection' % (
    ...
    def fix_cpu_type(self, cpu_type):
        ...
        if cpu_type.startswith('STM32'):
            ...
            return cpu_type
        raise pylib.stlinkex.StlinkException('"%s" is not STM32 family' % cpu_type)
        ...
        for detected_cpu in self._mcus:
            for expected_cpu in expected_cpus:
                ...
                if detected_cpu['type'].startswith(expected_cpu):
                    ...
                    break
        if not cpus:
            raise pylib.stlinkex.StlinkException('Connected CPU is not %s but detected is %s %s' % (
                ','.join(expected_cpus),
    ...
    def load_driver(self):
        ...
        if flash_driver == 'STM32FP':
            self._driver = pylib.stm32fp.Stm32FP(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32FPXL':
            self._driver = pylib.stm32fp.Stm32FPXL(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32FS':
            self._driver = pylib.stm32fs.Stm32FS(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32L0':
            self._driver = pylib.stm32l0.Stm32L0(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32L4':
            self._driver = pylib.stm32l4.Stm32L4(self._stlink, dbg=self._dbg)
        elif flash_driver == 'STM32H7':
            self._driver = pylib.stm32h7.Stm32H7(self._stlink, dbg=self._dbg)
        else:
            self._driver = self._core

    def detect_cpu(self, expected_cpus, unmount=False):
        self._connector = pylib.stlinkusb.StlinkUsbConnector(dbg=self._dbg, serial=self._serial, index = self._index)
        if unmount:
            self._connector.unmount_discovery()
        self._stlink = pylib.stlinkv2.Stlink(self._connector, dbg=self._dbg)
        ...
        if self._stlink.coreid == 0:
            raise pylib.stlinkex.StlinkException('Not connected to CPU')
        self._core = pylib.stm32.Stm32(self._stlink, dbg=self._dbg)
    ...
    def read_file(self, filename):
        ...
        with open(filename, 'rb') as f:
            ...
            return [(None, data)]
        raise pylib.stlinkex.StlinkException("Error reading file")
    ...
    def cmd_read(self, params):
        ...
        if cmd == 'flash':
            ...
            raise pylib.stlinkex.StlinkExceptionBadParam()
        data = self._driver.get_mem(addr, size)
    ...
    def cmd_set(self, params):
        ...
        params = params[1:]
        if not params:
            raise pylib.stlinkex.StlinkExceptionBadParam('Missing argument')
        data = int(params[0], 0)
    ...
    def cmd_write(self, params):
        ...
        if len(mem) == 1 and mem[0][0] is None:
            data = mem[0][1]
            if len(params) != 1:
                raise pylib.stlinkex.StlinkExceptionBadParam('Address is not set')
            if params[0] == 'sram':
                addr = self._driver.SRAM_START
                if len(data) > self._sram_size * 1024:
                    raise pylib.stlinkex.StlinkExceptionBadParam('Data are bigger than SRAM')
            else:
                addr = int(params[0], 0)
            self._driver.set_mem(addr, data)
            return
        if params:
            raise pylib.stlinkex.StlinkException('Address for write is set by file')
    ...
    def cmd_flash(self, params):
        ...
        if params and params[0] == 'verify':
            verify = True
            params = params[1:]
        start_addr = pylib.stm32.Stm32.FLASH_START
        if len(mem) == 1 and mem[0][0] is None:
            if params:
                start_addr = int(params[0], 0)
                params = params[1:]
        if params:
            raise pylib.stlinkex.StlinkExceptionBadParam('Address for write is set by file')
        for addr, data in mem:
            if addr is None:
                addr = start_addr
            if write:
    ...
    def cmd(self, param):
        # if the file is from an absolute path, combine the ":"
        if(param[-1].endswith("bin")) and param[-2].startswith("0x") is False:
            param[-2:] = [':'.join(param[-2:])]
    ...
    def start(self, argv):
        ...
        self._dbg = pylib.dbg.Dbg(args.verbosity)
        ...
        try:
            ...
            if args.action and self._driver is None:
                raise pylib.stlinkex.StlinkExceptionCpuNotSelected()
            for action in args.action:
                self._dbg.verbose('CMD: %s' % action)
                try:
                    self.cmd(action.split(':'))
                except pylib.stlinkex.StlinkExceptionBadParam as e:
                    raise e.set_cmd(action)
        except (pylib.stlinkex.StlinkExceptionBadParam, pylib.stlinkex.StlinkException) as e:
            self._dbg.error(e)
            runtime_status = 1
        ...
        if self._stlink:
            ...
            except pylib.stlinkex.StlinkException as e:
                self._dbg.error(e)
                runtime_status = 1
