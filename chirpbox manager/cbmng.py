...
10) \"python cbmng.py -colver [com_port]\" to obtain the daemon version.\n\
...
def main(argv):
	if(((argv[1] == "version") or (argv[1] == "-v")) and (len(argv) == 2)):
		...
		exit(0)
	elif(((argv[1] == "collect_version") or (argv[1] == "-colver")) and (len(argv) == 3)):
		if((cbmng_exp_start.check_finished() == True) and (cbmng_exp_start.is_running() == False)):
			cbmng_exp_start.collect_version(argv[2])
		exit(0)
