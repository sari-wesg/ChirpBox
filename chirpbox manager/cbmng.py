...
6) \"python cbmng.py -dissem [upgrading_daemon] [ver_hash] [com_port]\" to disseminate the file, e.g., the firmware. A patch for daemon is generated if upgrading_daemon is 1.\n\
...
9) \"python cbmng.py -coltopo [using_pos] [used_SF] [com_port]\" to obtain the topology. If using_pos is 0, the layout of topology is generated randomly; if using_pos is 1, the previously generated layout is used; if using_pos is 2, a specific layout for SARI is used.\n\
10) \"python cbmng.py -colver [used_SF] [com_port]\" to obtain the daemon version.\n\
11) \"python cbmng.py -assignsnf [com_port]\" to assign a node to work as a sniffer at a given channel (KHz). Sniffers and channels are given in the methodology file.\n\
12) \"python cbmng.py -upgrade [filename] [ver_hash] [com_port]\" to upgrade the daemon. The filename is the updated daemon.bin.")
...
def main(argv):
	if(((argv[1] == "version") or (argv[1] == "-v")) and (len(argv) == 2)):
		...
		exit(0)
	elif(((argv[1] == "collect_topology") or (argv[1] == "-coltopo")) and (len(argv) == 5)):
		if((cbmng_exp_start.check_finished() == True) and (cbmng_exp_start.is_running() == False)):
			cbmng_exp_start.collect_topology(argv[4], int(argv[2]), int(argv[3]))
		exit(0)
	elif(((argv[1] == "collect_version") or (argv[1] == "-colver")) and (len(argv) == 4)):
		if((cbmng_exp_start.check_finished() == True) and (cbmng_exp_start.is_running() == False)):
			cbmng_exp_start.collect_version(argv[3], int(argv[2]))
		exit(0)
	elif(((argv[1] == "disseminate") or (argv[1] == "-dissem")) and (len(argv) == 5)):
		if(cbmng_exp_start.check() == True):
			cbmng_exp_start.disseminate(argv[4], int(argv[2]), argv[3])
		exit(0)
	elif(((argv[1] == "upgrade") or (argv[1] == "-upgrade")) and (len(argv) == 5)):
		if(expfirmapp.experiment_firmware(argv[2]) == True):
			expfirmapp.read_configuration()
		...
		if(expmethapp.experiment_methodology("tmp.json") == True):
			expmethapp.read_configuration()
		cbmng_exp_start.disseminate(argv[4], 1, argv[3])
		cbmng_exp_start.start(argv[4], 0)
		exit(0)		
