...
6) \"python cbmng.py -dissem [upgrading_daemon] [ver_hash] [payload_len] [generate_size_in_round] [used_SF] [com_port] [bitmap] [slot_num][dissem_back_sf][dissem_back_slot][used_tp] \" to disseminate the file, e.g., the firmware. A patch for daemon is generated if upgrading_daemon is 1.\n\
7) \"python cbmng.py -coldata [payload_len] [used_SF] [com_port] [slot_num] [used_tp] \" to collect the results in the given area of the flash (from [start_addr] to [end_addr]). These addresses are assigned in the methodology file.\n\
...
12) \"python cbmng.py -upgrade [filename] [ver_hash] [payload_len] [generate_size_in_round] [used_SF] [com_port] [bitmap] [slot_num][dissem_back_sf][dissem_back_slot][used_tp]\" to upgrade the daemon. The filename is the updated daemon.bin.")
	...
	if(((argv[1] == "version") or (argv[1] == "-v")) and (len(argv) == 2)):
		...
		if(expmethapp.experiment_methodology(argv[2]) == True):
			expmethapp.read_configuration()
	 	 	...
	 	 	exit(0)
	elif(((argv[1] == "experiment_start") or (argv[1] == "-start")) and (len(argv) == 8)):
	 	if(cbmng_exp_start.check() == True):
	 		cbmng_exp_start.start(argv[5], int(argv[2]), argv[3], int(argv[4]), argv[6], int(argv[7]))
	 	exit(0)
		...
		# exit(0)
	elif(((argv[1] == "collect_data") or (argv[1] == "-coldata")) and (len(argv) == 7)):
		if((cbmng_exp_start.check_finished() == True) and (cbmng_exp_start.is_running() == False)):
			cbmng_exp_start.collect_data(argv[4], int(argv[2]), int(argv[3]), int(argv[5]), int(argv[6]))
		# exit(0)
		...
		# exit(0)
	elif(((argv[1] == "disseminate") or (argv[1] == "-dissem")) and (len(argv) == 13)):
		if(cbmng_exp_start.check() == True):
			cbmng_exp_start.disseminate(argv[7], int(argv[2]), argv[3], int(argv[4]), int(argv[6]), int(argv[5]), argv[8], int(argv[9]), int(argv[10]), int(argv[11]), int(argv[12]))
		# exit(0)
	elif(((argv[1] == "upgrade") or (argv[1] == "-upgrade")) and (len(argv) == 13)):
		if(expfirmapp.experiment_firmware(argv[2]) == True):
			expfirmapp.read_configuration()
		...
		if(expmethapp.experiment_methodology("tmp.json") == True):
			expmethapp.read_configuration()
		cbmng_exp_start.disseminate(argv[7], 1, argv[3], int(argv[4]), int(argv[6]), int(argv[5]), argv[8], int(argv[9]), int(argv[10]), int(argv[11]), int(argv[12]))
